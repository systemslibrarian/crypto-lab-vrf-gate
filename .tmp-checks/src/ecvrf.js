import { bigintToBytes, bytesToBigInt, concatBytes, equalBytes, numberToBytes, utf8ToBytes, } from './utils/bytes.js';
import { addPoints, bytesToPoint, isOnCurve, mod, P256_G, P256_N, P256_P, pointToBytes, scalarFromBytes, scalarMultiply, scalarToBytes, sqrtModP, } from './utils/p256.js';
...
async function exportPrivateScalar(privateKey) {
...
async function importRawPublicKey(pointBytes) {
    return crypto.subtle.importKey('raw', toArrayBuffer(pointBytes), { name: 'ECDH', namedCurve: 'P-256' }, true, []);
}
async function deriveEcdhXCoordinate(privateKey, publicPointBytes) {
    const publicKey = await importRawPublicKey(publicPointBytes);
    const bits = await crypto.subtle.deriveBits({ name: 'ECDH', public: publicKey }, privateKey, 256);
    return new Uint8Array(bits);
}
async function crossCheckXCoordinate(privateKey, publicPointBytes, expectedPoint) {
 ...
 */
export async function vrfKeyGen() {
 ...
 */
export async function hashToCurve(pkBytes, alpha) {
 ...
 */
export async function challengeGeneration(H, Gamma, U, V) {
 ...
 */
export async function proofToHash(gamma) {
 ...
 */
export async function vrfProve(keyPair, alpha) {
 ...
 */
export async function vrfVerify(publicKeyBytes, alpha, output) {
    try {
        ...
        const UPoint = requirePoint(addPoints(scalarMultiply(sScalar, P256_G), scalarMultiply(cScalar, publicKeyPoint)), 'U');
        const VPoint = requirePoint(addPoints(scalarMultiply(sScalar, HPoint), scalarMultiply(cScalar, gammaPoint)), 'V');
        const expectedChallenge = await challengeGeneration(H, output.proof.gamma, pointToBytes(UPoint), pointToBytes(VPoint));
